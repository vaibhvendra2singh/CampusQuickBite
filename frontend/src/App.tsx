...
import OrderLiveStatus from './pages/student/OrderLiveStatus';
...
import RestaurantsPage from './pages/student/RestaurantsPage';
import GameHub from './components/student/GameHub';
...
const Header = ({ darkMode, setDarkMode }: { darkMode: boolean, setDarkMode: (v: boolean) => void }) => {
    ...
    return (
        <header className="sticky top-0 z-[60] w-full backdrop-blur-2xl bg-white/80 dark:bg-slate-950/80 py-4 px-6 border-b border-slate-200/40 dark:border-slate-800/40 transition-all duration-150">
            <nav className="max-w-7xl mx-auto flex items-center justify-between">
                <Link to="/" className="group flex items-center space-x-4">
                    <div className="w-11 h-11 bg-gradient-to-br from-brand-500 to-brand-700 rounded-2xl flex items-center justify-center font-black text-white shadow-xl shadow-brand-500/20 group-hover:-rotate-3 transition-all duration-150">
                        <span className="text-xl tracking-tighter">CB</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-none group-hover:text-brand-500 transition-colors">CampusBite</span>
                        <span className="text-[10px] font-black text-brand-600 dark:text-brand-400 uppercase tracking-widest mt-1 opacity-80 group-hover:opacity-100 transition-all">Freshly Served</span>
                    </div>
                </Link>
                ...
                <div className="flex items-center space-x-4">
                    {user?.role === 'STUDENT' && (
                        <div className="hidden lg:flex items-center space-x-1.5 p-1.5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800/50">
                            <Link to="/" className="flex items-center space-x-2 px-5 py-2.5 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:text-brand-500 transition-all font-bold text-sm">
                                <FiServer className="w-4 h-4 opacity-70" /><span>Board</span>
                            </Link>
                            <Link to="/restaurants" className="flex items-center space-x-2 px-5 py-2.5 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:text-brand-500 transition-all font-bold text-sm">
                                <FiCompass className="w-4 h-4 opacity-70" /><span>Explore</span>
                            </Link>
                            <Link to="/orders/history" className="flex items-center space-x-2 px-5 py-2.5 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-slate-600 dark:text-slate-400 hover:text-brand-500 transition-all font-bold text-sm">
                                <FiShoppingBag className="w-4 h-4 opacity-70" /><span>History</span>
                            </Link>
                            <Link to="/leaderboard" className="flex items-center space-x-2 px-5 py-2.5 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-amber-500 hover:text-amber-600 transition-all font-bold text-sm">
                                <FiAward className="w-4 h-4" /><span>Elite</span>
                            </Link>
                            <Link to="/cart" className="flex items-center space-x-2 px-6 py-2.5 bg-brand-500 hover:bg-brand-600 rounded-xl text-white transition-all font-black text-sm shadow-xl shadow-brand-500/25 ml-2 relative">
                    ...
                    <div className="flex items-center space-x-2 bg-white/50 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800/50 p-1.5 rounded-2xl shadow-sm">
                        <button onClick={() => setDarkMode(!darkMode)} className="p-3 text-slate-500 hover:text-brand-500 hover:bg-brand-50/50 dark:hover:bg-slate-800 rounded-xl transition-all" title="Shift View">
                            {darkMode ? <FiSun className="w-5 h-5" /> : <FiMoon className="w-5 h-5" />}
                        </button>

                        {user ? (
                            <div className="flex items-center space-x-2 pl-2 border-l border-slate-200/50 dark:border-slate-800/50">
                                <button onClick={() => setIsProfileOpen(true)} className="flex items-center space-x-3 px-3 py-2 hover:bg-slate-100/50 dark:hover:bg-slate-800/50 transition-all rounded-xl">
                                    <div className="w-9 h-9 rounded-2xl overflow-hidden bg-brand-100 dark:bg-brand-900/30 border-2 border-brand-200 dark:border-brand-800 flex items-center justify-center shadow-inner group">
                                        {user?.profilePic ? (
                                            <img src={user.profilePic} alt={user?.name || ''} className="w-full h-full object-cover transition-all duration-150" />
                                        ) : (
                                ...
                                <button onClick={logout} className="p-3 text-red-500 hover:text-white hover:bg-red-500 rounded-xl transition-all" title="Exit">
                                    <FiLogOut className="w-5 h-5" />
                                </button>
                            </div>
                        ) : (
                            <Link to="/login" className="bg-slate-950 dark:bg-white text-white dark:text-slate-950 hover:bg-brand-600 dark:hover:bg-brand-500 hover:text-white dark:hover:text-white px-7 py-3 rounded-2xl font-black text-sm transition-all shadow-xl shadow-slate-900/10 ml-2">Join In</Link>
                        )}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-20">
                        ...
                        <div>
                            ...
                            <ul className="space-y-4 text-base text-slate-300 font-bold">
                                <li><Link to="/" className="hover:text-white inline-block transition-all duration-150">Daily Feed</Link></li>
                                <li><Link to="/orders/history" className="hover:text-white inline-block transition-all duration-150">History</Link></li>
                                <li><Link to="/leaderboard" className="hover:text-white inline-block transition-all duration-150">ELITE Board</Link></li>
                            </ul>
                        </div>
                        ...
                        <div>
                            ...
                            <ul className="space-y-4 text-base text-slate-300 font-bold">
                                <li><Link to="/help" className="hover:text-white inline-block transition-all duration-150">Get Help</Link></li>
                                <li><Link to="/contact" className="hover:text-white inline-block transition-all duration-150">Talk to us</Link></li>
                                <li><Link to="/privacy" className="hover:text-white inline-block transition-all duration-150">Privacy</Link></li>
                                <li><Link to="/terms" className="hover:text-white inline-block transition-all duration-150">Rules of Play</Link></li>
                            </ul>
                        </div>
                    </div>
...
const ProtectedRoute = ({ children, allowedRoles }: { children: React.ReactNode, allowedRoles?: string[] }) => {
    ...
    if (isLoading) return (
        <div className="h-screen bg-[var(--bg-primary)] flex flex-col items-center justify-center space-y-4">
...
function App() {
    ...
    if (isLoading) {
        return (
            <div className="h-screen bg-[var(--bg-primary)] flex flex-col items-center justify-center space-y-5">
                <div className="w-16 h-16 bg-brand-500 rounded-2xl flex items-center justify-center shadow-lg">
                    <FiZap className="text-white w-8 h-8" />
                </div>
                <div className="flex flex-col items-center">
                            ...
                            <Route path="/unauthorized" element={
                                <div className="min-h-[70vh] flex flex-col items-center justify-center animate-none px-6">
                                    ...
                                    <div className="flex space-x-4 mt-8">
                                        ...
                                        <button onClick={() => { logout(); window.location.href = '/login'; }} className="btn-secondary px-8 py-3 text-red-500 border-red-200 hover:bg-red-50 dark:border-red-500/20 dark:hover:bg-red-500/10">Sign Out</button>
                                    </div>
                                </div>
                            } />
